...
bool Doxygen::writeMain()
{
    ...
    auto msgHeader = common::scopeFor(ns, common::fieldNamespaceStr() + m_db.getMessageHeaderType());
    auto msgHeaderMembers = common::scopeFor(ns, common::fieldNamespaceStr() + m_db.getMessageHeaderType() + common::memembersSuffixStr());
    auto msgHeaderMembersVersion = msgHeaderMembers + "::" + common::versionStr();
    auto msgHeaderFrame = common::scopeFor(ns, common::messageHeaderFrameStr());
    auto openFrameHeader = common::scopeFor(ns, common::builtinNamespaceStr() + common::openFramingHeaderStr());
    auto openFramingHeaderFrame = common::scopeFor(ns, common::openFramingHeaderFrameStr());
    auto defaultOptions = common::scopeFor(ns, common::defaultOptionsStr());
    auto defaultOptionsFieldsNs = defaultOptions + "::" + common::fieldNamespaceNameStr();
    auto defaultOptionsMsgHeaderMembers = defaultOptionsFieldsNs + "::" + m_db.getMessageHeaderType() + common::memembersSuffixStr();
    auto defaultOptionsMsgHeaderVersion = defaultOptionsMsgHeaderMembers + "::" + common::versionStr();
    ...
    out << "/// \\mainpage " << m_db.getPackageName() << " Binary Protocol\n"
           ...
           "/// which can be used for easier definition of the custom handler one. The\n"
           ...
           "///     using someField = " << builtinNs << "::uint8<...>;\n"
           "///\n"
           ...
           "/// as alias to " << builtinNs << "::groupList common definition. The latter implements the\n"
           ...
           "/// between the fields. It is implemented as alias to " << builtinNs << "::pad\n"
           "/// field, which is implemented as \\b comms::field::ArrayList of raw data.\n"
           "///\n"
           "/// \\section main_page_schema_version Schema Version\n"
           "/// The \\b SBE schema allows adding new fields (with non-zero\n"
           "/// \\b sinceVersion attribute) in new versions of the\n"
           "/// schema.\n"
           "/// \\code\n"
           "/// <field name=\"someNew\" id=\"100\" type=\"uint16\" sinceVersion=\"1\"/>\n"
           "/// \\endcode\n"
           "/// Values of such fields may or may not exist in the communicated data,\n"
           "/// depending on the reported schema version of the received message. The\n"
           "/// \\b sbe2comms compiler defines such field in two stages. First, it defines\n"
           "/// it as usual field, but with \\b *Field suffix in its name. Then, it wraps\n"
           "/// it in \\b comms::field::Optional.\n"
           "/// \\code\n"
           "/// template <typename... TOpt>\n"
           "/// class someNewField : public comms::field::IntValue<...>\n"
           "/// {\n"
           "///     ...\n"
           "/// };\n"
           "///\n"
           "/// template <typename... TOpt>\n"
           "/// struct someNew : public\n"
           ...
           "/// };\n"
           "/// \\endcode\n"
           "/// \\b NOTE, that \\b comms::field::Optional class wraps the field and has\n"
           "/// the following public interface types and functions:\n"
           "/// \\code\n"
           ...
           "///     using Field = ...;\n"
           "///\n"
           "///     // Access to the wrapped field\n"
           "///     Field& field();\n"
           "///     const Field& field() const\n"
           "///\n"
           "///     // Get/set mode\n"
           "///     comms::field::OptionalMode getMode() const;\n"
           "///     void setMode(comms::field::OptionalMode mode);\n"
           "///\n"
           "///     // Simple wrappers to get/set mode functions above\n"
           ...
           "/// };\n"
           "/// \\endcode\n"
           "/// When version of the incoming message is known, the mode of such field is\n"
           "/// updated accordingly. Then, the \\b read operation that follows will either\n"
           "/// read value properly or will do nothing without updating the iterator.\n"
           "/// Processing of such field may look like this:\n"
           "/// \\code\n"
           "/// void processOptionalField(someNew<>& f)\n"
           "/// {\n"
           "///     std::uint16_t value = 0; // some default value\n"
           "///     if (f.doesExist()) {\n"
           "///         value = f.field().value(); // get the real value\n"
           "///     }\n"
           "///     ... // do something with value\n"
           "/// }\n"
           "/// \\endcode\n"
           "/// Please pay attention to the differences between the optional fields\n"
           "/// defined by the schema (with \\b presence=\"optional\" attribute) and\n"
           "/// fields that implemented using \\b comms::field::Optional. These are\n"
           "/// two completely different things. The value of the former is always\n"
           "/// transmitted on the I/O link, it just has a special \\b null value. The\n"
           "/// latter, on the other hand, wraps fields that were introduced in later\n"
           "/// versions of the schema, values of which may or may not be transmitted over\n"
           "/// the I/O link.\n"
           "///\n"
           ...
           "/// \\code\n"
           "/// using MyInterface = " << interfaceType << "<...>;\n"
           "/// using MyTransport = " << msgHeaderFrame << "<MyInterface, " << allMessages << "<MyInterface> >;\n"
           "/// \\endcode\n"
           "/// Note, that messages inside the provided \\b std::tuple must be sorted\n"
           "/// according by their numeric IDs. There is \\ref " << allMessages << " type\n"
           ...
           "/// The third template parameter of the \\ref " << msgHeaderFrame << " class is\n"
           "/// protocol customisation options, which can be used to update the\n"
           "/// default generated behaviour of \\ref " << msgHeader << " member fields.\n"
           ...
           "/// using fourth template parameter of \\ref " << msgHeaderFrame << ".\n"
           ...
           "/// as a fourth template parameter to \\ref " << msgHeaderFrame << ".\n"
           "///\n"
           ...
           "/// The \\b sbe2comms compiler generates " << openFrameHeader << "\n"
           ...
           "///\n"
           "/// Similar to \\ref " << msgHeaderFrame << ", there is also \\ref\n"
           "/// " << openFramingHeaderFrame << ", defined in the same TransportFrame.h file.\n"
           ...
           "/// member function of \\ref " << openFramingHeaderFrame << " will return\n"
           ...
           "/// \\code\n"
           "/// using MyTransport = " << openFramingHeaderFrame << "<MyInterface, mine::AllMessages<MyInterface> >;\n"
           ...
           "/// values. The \\b sbe2comms compiler generates \\ref " << idType << " enum\n"
           "/// for message identification purpose. The \\b " << common::templateIdStr() << " member\n"
           "/// field of \\ref " << msgHeader << " is used to carry message ID information.\n"
           ...
           "/// type for message IDs and make \\b " << common::templateIdStr() << " member\n"
           "/// field of \\ref " << msgHeader << " to be a \\b \\<ref\\> to it.\n"
           "/// \\code\n"
           ...
           "/// \\b " << common::templateIdStr() << " to extend \\b comms::field::EnumValue\n"
           "/// \\code\n"
           "/// template <typename... TOpt>\n"
           ...
           "/// Every message class from \\ref " << messageNs << " namespace receives a second\n"
           "/// template parameter, which defaults to \\ref " << defaultOptions << " from\n"
           "/// " << common::defaultOptionsFileName() << " header file. Please take a look inside. The inner\n"
           "/// scopes of various types a mapped one-to-one with scopes of fields\n"
           "/// in \\ref " << ns << " namespace. These inner types are passed as extra options\n"
           "/// to various fields. For example \\ref " << defaultOptionsMsgHeaderVersion << "\n"
           "/// type is passed as extra options to \\ref " << msgHeaderMembersVersion << "\n"
           "/// field.\n"
           "///\n"
           "/// All the types inside \\ref " << defaultOptions << " are defined to be\n"
           ...
           "/// passed as a template parameter instead of \\ref " << defaultOptions << ". The\n"
           "/// way is to inherit from \\ref " << defaultOptions << " and override types for\n"
           ...
           "/// only version \\b " << m_db.getSchemaVersion() << " of the schema and should fail the read operation if\n"
           "/// the actual value is not as expected. Below is how custom protocol options\n"
           "/// will look like:\n"
           "/// \\code\n"
           "/// struct MyOptions : public " << defaultOptions << "\n"
           "/// {\n"
           "///     struct field : public " << defaultOptionsFieldsNs << "\n"
           "///     {\n"
           "///         struct " << m_db.getMessageHeaderType() << common::memembersSuffixStr() << " : public " << defaultOptionsMsgHeaderMembers << "\n"
           "///         {\n"
           "///             using version =\n"
           "///                 std::tuple<\n"
           "///                     comms::option::ValidNumValueOverride<" << m_db.getSchemaVersion() << ">,\n"
           ...
           "/// default valid range used in the definition of \\ref " << msgHeaderMembersVersion << ".\n"
           "///\n"
